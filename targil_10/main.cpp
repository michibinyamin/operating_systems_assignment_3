...
void* handle_condition(void* arg){
    while (true) {
        ...
        if(over_fifty){
            std::cout << "At Least 50 percent of the graph belongs to the same SCC\n";
        }else{
            std::cout << "At Least 50 percent of the graph no longer belongs to the same SCC\n";
        }
        
        pthread_cond_signal(&cond); // Signal producer that it can produce more
...
int main() {
    ...
    // Create a thread which checks for condition of 50 percent graph
    pthread_t tid;
    int result = pthread_create(&tid, NULL, handle_condition, NULL);
    if (result != 0) {
